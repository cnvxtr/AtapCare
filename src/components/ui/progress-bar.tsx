...
export type ProgressBarProps = {
  /** 0–1 untuk progres nyata; null = indeterminate (animasi berjalan). */
  value: number | null;
  max?: number;
  label?: string;
  pendingLabel?: string;
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div
        ...
        aria-valuemax={max}
        {...measured}
        className="mt-2 rounded-[4px] bg-muted p-[2px] shadow-[inset_0_1px_2px_rgba(0,0,0,0.1)]"
      >
        <div className="relative h-[8px] overflow-hidden rounded-[2px]">
          <motion.span
            aria-hidden
            className="absolute inset-0 block origin-left rounded-[2px] bg-foreground"
            initial={false}
          ...
          {indeterminate && !reduced ? (
            <motion.span
              aria-hidden
              className="absolute inset-y-0 left-0 block w-2/5 rounded-[2px] bg-foreground"
