-- Migration 61: Atribusi rating ke helpdesk yang VALIDASI (NEW → OPEN).
-- Rating itu sendiri (rating/review) sudah ada di tickets (migration 45).
-- Di sini kita rekam SIAPA helpdesk yang melayani customer = helpdesk yang
-- klik Validasi di awal, agar leaderboard rating memakai id user (bukan nama).

-- ── 1. Kolom atribusi di tickets ──
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS rating_helpdesk_user_id uuid REFERENCES users(id) ON DELETE SET NULL;
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS rating_helpdesk_name text;

-- ── 2. update_ticket_status (kumulatif, pola migration 49) ──
-- Tambahan: saat NEW→OPEN, rekat helpdesk yang validasi (auth.uid() aktor).
